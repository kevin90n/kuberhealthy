...
func NewStateReflector(khStateClient *khstatev1.KHStateV1Client, stateCRDResource string, namespaces string) *StateReflector {
	sr := StateReflector{}
	sr.reflectorSigChan = make(chan struct{})
	sr.resyncPeriod = time.Minute * 5

	// structure the reflector and its required elements
	khStateListWatch := cache.NewListWatchFromClient(khStateClient.RESTClient(), stateCRDResource, namespaces, fields.Everything())
...
func (sr *StateReflector) CurrentStatus() health.State {
	...
	for i, khStateUndefined := range khStateList {
		...
		switch khState.Spec.KHWorkload {
