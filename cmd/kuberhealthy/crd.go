...
func setCheckStateResource(ctx context.Context, checkName string, checkNamespace string, state khstatev1.WorkloadDetails) error {
	...
	// int found within
	existingState, err := khStateClient.Namespace(checkNamespace).Get(ctx, name, metav1.GetOptions{})
	if err != nil {
	...
	unstructuredKHState, err := runtime.DefaultUnstructuredConverter.ToUnstructured(khState)
	if err != nil {
		return errors.New("Error converting check state to unstructured resource: " + name + " " + err.Error())
	}

	// convert the unstructured map to an unstructured struct
	unstructuredCheck := &unstructured.Unstructured{Object: unstructuredKHState}
	_, err = khStateClient.Namespace(checkNamespace).Update(ctx, unstructuredCheck, metav1.UpdateOptions{})
	return err
}
...
func ensureStateResourceExists(ctx context.Context, checkName string, checkNamespace string, workload khstatev1.KHWorkload) error {
	...
	state, err := khStateClient.Namespace(checkNamespace).Get(ctx, name, metav1.GetOptions{})
	if err != nil {
		if k8sErrors.IsNotFound(err) || strings.Contains(err.Error(), "not found") {
			...
			_, err := khStateClient.Namespace(checkNamespace).Create(ctx, initialState, metav1.CreateOptions{})
			if err != nil {
...
func getCheckState(ctx context.Context, c *external.Checker) (khstatev1.WorkloadDetails, error) {
	...
	khstate, err := khStateClient.Namespace(c.CheckNamespace()).Get(ctx, name, metav1.GetOptions{})
	if err != nil {
...
func getJobState(ctx context.Context, j *external.Checker) (khstatev1.WorkloadDetails, error) {
	...
	log.Debugln("Retrieving khstate custom resource for:", name)
	khstate, err := khStateClient.Namespace(j.Namespace).Get(ctx, name, metav1.GetOptions{})
	if err != nil {
		return state, errors.New("Error retrieving custom khstate resource: " + name + " " + err.Error())
	}
	log.Debugln("Successfully retrieved khstate resource:", name)
...
func setJobPhase(ctx context.Context, jobName string, jobNamespace string, jobPhase khjobv1.JobPhase) error {
	...
	kj, err := khJobClient.Namespace(jobNamespace).Get(ctx, jobName, metav1.GetOptions{})
	if err != nil {
	...
	_, err = khJobClient.Namespace(jobNamespace).Update(ctx, &updatedJob, metav1.UpdateOptions{})
	return err
}
